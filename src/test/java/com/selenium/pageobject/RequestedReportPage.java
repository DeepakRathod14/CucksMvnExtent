...
import com.utility.constant.CmeSelenium.ColumnName;
import com.utility.constant.CmeSelenium.Sorting;
import com.utility.constant.CmeSelenium.StatusIcon;
...
public class RequestedReportPage extends BasePageObject<T> {
	...
	public boolean verifyTabsVisibility(String tabName, String action) {
		if (tabName.equalsIgnoreCase(CmeSelenium.REQUESTED_REPORTS.toString().replace("_", " ")) && action.equalsIgnoreCase(CmeSelenium.ENABLE)) {
			return isEnabled(By.id(""));
		} else if (tabName.equalsIgnoreCase(CmeSelenium.REQUESTED_REPORTS.toString().replace("_", " "))
				&& action.equalsIgnoreCase(CmeSelenium.DISABLE)) {
			return !(isEnabled(By.id("")));
		} else if (tabName.equalsIgnoreCase(CmeSelenium.SCHEDULE_REPORTS.toString().replace("_", " "))
				&& action.equalsIgnoreCase(CmeSelenium.ENABLE)) {
			return (isEnabled(By.id("")));
		} else if (tabName.equalsIgnoreCase(CmeSelenium.SCHEDULE_REPORTS.toString().replace("_", " "))
				&& action.equalsIgnoreCase(CmeSelenium.DISABLE)) {
			return !(isEnabled(By.id("")));
	...
	{
		int size = selenium.findElements(By.xpath("")).size();
		int columnNumber = 0;
		String tdText="";
	...
	public boolean verifyGridGlobleSearchRecored(String columnName, String recordDetail) {
		...
		List<WebElement> allTD = selenium.findElements(By.xpath(""));
	...
	{
		return message.equalsIgnoreCase(selenium.switchTo().alert().getText().trim());
	}
	...
	{
		return pageTitle.equalsIgnoreCase(selenium.getTitle());
	}
	/**
	 * Gets the list.
	 * @author GS-1496
	...
	private ArrayList<String> getList(int columnNumber) {
		List<WebElement> ElementsList = selenium.findElements(By.xpath(""));
